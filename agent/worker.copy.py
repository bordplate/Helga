...
def update_configuration(redis: Redis):
    configuration["epsilon"] = float(redis.get("rac1.hoverboard.epsilon")) if redis.get("rac1.hoverboard.epsilon") is not None else 1.0
    configuration["min_epsilon"] = float(redis.get("rac1.hoverboard.min_epsilon")) if redis.get("rac1.hoverboard.min_epsilon") is not None else 0.005
...
def start_worker():
    ...
    agent = Agent(gamma=0.99, epsilon=configuration["epsilon"], batch_size=0, n_actions=4, eps_end=configuration["min_epsilon"],
    ...
    while True:
        ...
        while True:
            ...
            redis.publish("rac1.hoverboard.replay_buffer", data)
            ...
            if steps % 5 == 0:
                print(f"Score: %6.2f    dist: %06.2f checkpoint: %d      " % (
                    accumulated_reward,
        ...
        redis.rpush("rac1.hoverboard.avg_scores", accumulated_reward)
