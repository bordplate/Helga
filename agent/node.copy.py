...
def listen_for_messages(redis: Redis, replay_buffer: PrioritizedReplayBuffer):
    ...
    pubsub = redis.pubsub()
    pubsub.subscribe("rac1.hoverboard.replay_buffer")
...
def start():
    ...
    # Create an agent
    agent = Agent(gamma=0.99, epsilon=1.0, batch_size=batch_size, n_actions=13, eps_end=0.005,
    ...
    if args.model:
        ...
        redis.set("rac1.hoverboard.model", model)
        redis.set("rac1.hoverboard.model_timestamp", time.time())

    existing_model = redis.get("rac1.hoverboard.model") if args.model is None else None
    if existing_model is not None:
        agent.epsilon = float(redis.get("rac1.hoverboard.epsilon"))
        agent.Q_eval.load_state_dict(pickle.loads(existing_model))

        # Load optimizer if it exists
        existing_optimizer = redis.get("rac1.hoverboard.optimizer")
    ...
    if args.wandb:
        current_run_id = redis.get("rac1.hoverboard.wandb_run_id")
        current_run_id = current_run_id.decode() if current_run_id is not None else None

        wandb.init(project="rac1-hoverboard", id=current_run_id, config={
        ...
        redis.set("rac1.hoverboard.wandb_run_id", wandb.run.id)
    ...
    while True:
        ...
        if steps % 20 == 0:
            if commit:
                ...
                redis.set("rac1.hoverboard.model", model)
                redis.set("rac1.hoverboard.optimizer", optimizer)
                redis.set("rac1.hoverboard.model_timestamp", time.time())

                redis.set("rac1.hoverboard.epsilon", agent.epsilon)
        ...
        if steps % 200 == 0:
            ...
            scores = redis.lrange("rac1.hoverboard.avg_scores", -100, -1)
            ...
            if commit and steps % 10000 == 0:
                save_model(agent, f"models_bak/rac1_hoverboard_{int(steps/1000)}k.pth")
                print(f"Saved model to models_bak/rac1_hoverboard_{int(steps/1000)}k.pth")
