...
void PersistentStorage::persist() {
    int err = cellFsOpen(String::format("/dev_hdd0/game/NPEA00387/USRDIR/%s", filename.c_str()).c_str(), CELL_FS_O_RDWR|CELL_FS_O_TRUNC|CELL_FS_O_CREAT, &file_descriptor_, nullptr, 0);
...
void PersistentStorage::load() {
    int err = cellFsOpen(String::format("/dev_hdd0/game/NPEA00387/USRDIR/%s", filename.c_str()).c_str(), CELL_FS_O_RDONLY|CELL_FS_O_CREAT, &file_descriptor_, nullptr, 0);
